...
def process_row(row):
    try:
        ...
        fmt = row["output_format"].strip().lower()  # xử lý extra khoảng trắng
        ...
        send_slack(f"Success: {desc} | {drive_url}")
...
def generate_daily_report():
    ...
    plt.title("Today's Task Summary")
...
# Display the last 5 logs
df = pd.read_sql("SELECT * FROM logs", sqlite3.connect("task_log.db"))
print(df.tail())